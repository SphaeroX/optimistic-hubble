...
void printBanner() {
    Serial.println(F("\n========================================================"));
    Serial.println(F("  XIAO ESP32C3 - Smart Voice Recorder with BLE & IMU"));
    Serial.println(F("  Tap to Record -> Speak -> Tap to Send via BLE"));
    Serial.println(F("========================================================"));
}
...
void setup() {
    ...
    Serial.println(F("[1/4] Starting I2C & Probing 6-Axis IMU..."));
    ...
    Serial.println(F("[2/4] Initializing Stereo I2S Microphones..."));
    ...
    Serial.println(F("[3/4] Initializing Audio Recorder & Status LED..."));
    ...
    if (recOk) {
        ...
        recorder.blinkLed(3, 80); // Welcome blink
    } else {
        Serial.println(F("  [FAIL] Could not allocate Audio Buffer!"));
    }

    // 4. Initialize BLE GATT Server
    Serial.println(F("[4/4] Starting BLE GATT Server & Advertising..."));
    ...
    Serial.println(F("  3. Tap breadboard again to STOP & SEND to Companion App."));
    Serial.println(F("========================================================\n"));
}
...
void handleStopAndTransfer() {
    ...
    if (ble.isConnected()) {
        Serial.println(F("[BLE] Connected client detected. Transmitting voice clip..."));
        ble.transmitAudio(recorder.getBuffer(), recorder.getRecordedBytes(), recorder.getSampleRate());
    } else {
        Serial.println(F("[BLE] No BLE client connected. Audio stored in RAM ready for connection."));
        ble.updateState(STATE_IDLE);
    }
}

void loop() {
    // 1. Process Active Recording
    ...
    // 2. IDLE State: Monitor IMU for Start Tap
    ImuMetricData imuData;
    ...
    // 3. Periodic Telemetry (when Idle)
    ...
    if (now - lastTelemetryTime >= TELEMETRY_INTERVAL_MS) {
        ...
        Serial.printf("[BLE: %s] [MIC-L] %s RMS:%5.0f | [MIC-R] %s RMS:%5.0f | [ACC] X:%+4.2f Y:%+4.2f Z:%+4.2f\r",
