...
void WifiServerManager::handleRoot() {
    ...
    String html = "<!DOCTYPE html><html lang='de'><head><meta charset='utf-8'>"
                  ...
                  "<p>2-Kanal Stereo ADPCM &bull; Dual-Mic Beamforming & Universal WAV Export</p>"
    ...
    if (clips.empty()) {
        ...
        for (int i = clips.size() - 1; i >= 0; --i) {
            html += "<div class='clip'><div>"
                    ...
                    "<button onclick=\"playStereoAdpcm('/api/download?id=" + String(clips[i].id) + "')\">▶ Abspielen</button>"
    ...
    html += "</div></div><script>"
            ...
            "function decodeRawAdpcm(buf){"
            "  const raw = new Uint8Array(buf, 60);"
            "  const numFrames = raw.length;"
            "  const pcmL = new Int16Array(numFrames), pcmR = new Int16Array(numFrames);"
            ...
            "    pcmL[i] = dec(raw[i] & 15, true);"
            "    pcmR[i] = dec((raw[i] >> 4) & 15, false);"
            "  }"
            "  return { pcmL, pcmR, numFrames };"
            "}"
            "async function playStereoAdpcm(url){"
            "  if(!actx) actx = new (window.AudioContext||window.webkitAudioContext)();"
            "  const res = await fetch(url); const buf = await res.arrayBuffer();"
            "  const d = decodeRawAdpcm(buf);"
            "  const ab = actx.createBuffer(1, d.numFrames, 16000);"
            "  const out = ab.getChannelData(0);"
            "  for(let i=0; i<d.numFrames; i++) out[i] = ((d.pcmL[i] + d.pcmR[i]) / 2.0) / 32768.0;"
            ...
            "  const res = await fetch(url); const buf = await res.arrayBuffer();"
            "  const d = decodeRawAdpcm(buf);"
            "  const totalSize = 44 + (d.numFrames * 4);"
            ...
            "  v.setUint32(4, 36 + d.numFrames * 4, true);"
            ...
            "  v.setUint32(24, 16000, true); v.setUint32(28, 64000, true); v.setUint16(32, 4, true); v.setUint16(34, 16, true);"
            "  v.setUint8(36,0x64);v.setUint8(37,0x61);v.setUint8(38,0x74);v.setUint8(39,0x61);"
            "  v.setUint32(40, d.numFrames * 4, true);"
            "  let off = 44;"
            "  for(let i=0; i<d.numFrames; i++){"
            "    v.setInt16(off, d.pcmL[i], true); off += 2;"
            "    v.setInt16(off, d.pcmR[i], true); off += 2;"
            "  }"
            "  const blob = new Blob([v], {type:'audio/wav'});"
            "  const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'clip_' + id + '_stereo_pcm16.wav'; a.click();"
            "}"
            "</script></body></html>";
